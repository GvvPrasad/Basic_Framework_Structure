...
public class Salary_Comp {
	...
	public static void SalaryComp(WebDriver driver, XSSFSheet Sfile, ExtentTest setup, MultiScreenShot multiScreens, XSSFCell cell, String FilePath, XSSFWorkbook WBfile) throws IOException, InterruptedException {
		...
		for (int i=0; i<=Sfile.getLastRowNum(); i++) {
			...
			if (AlertMessagBlock.equalsIgnoreCase(AlertMessg)) {
				message = "Salary Comp Already exit";
				multiScreens.multiScreenShot(driver);
